...
module.exports = class SetConfigCommand extends Command {
      ...
      args: [
        {
          key: "configuration",
          prompt:
            "What do you want me to set? Choose `modlog, logging, reporting, welcoming, wm, wc, or blacklist`.",
          type: "string",
          oneOf: ["modlog", "logging", "reporting", "bl", "wordfilter", "welcoming", "wm", "wc"]
        },
        {
          key: "newvalue",
  ...
  async run(message, { configuration, newvalue }) {
    ...
    if (configuration === "wc") {
      await wchannel.set(message.guild.id, newvalue);
    }
    if (configuration === "wm") {
      await wmessage.set(message.guild.id, newvalue);
    }
    if (configuration === "welcoming") {
      await wmtoggle.set(message.guild.id, newvalue);
    }
    ...
    const embed = new MessageEmbed()
      ...
      .setDescription(
        stripIndents`I have set the configuration ${configuration} to ${newvalue} for guild ${message.guild.name} with the Discord Guild ID ${message.guild.id}.`
      );
    message.embed(embed);
  }
};
